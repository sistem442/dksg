...
<title>DKSG Foto Galerija </title>
...
// napisi izraz za upit u odnosu na datum prikazivanja galerije
$godina = date('Y');
$mesec = date('m');
$dan = date('d');
$danas = $godina.$mesec.$dan; 
//echo "danas je: ".$danas;
if($mesec > 6) $izraz = "WHERE (datum <= $danas AND datum >= ($danas - 600))";
else $izraz = "WHERE (datum <= $danas AND datum >=($danas - 9400))";
...
if(mysql_num_rows($result)==0){
	...
	echo "<div style='float:right; width:670px; color:black; font-size:18px; '> Foto galerija </DIV>			          <br />";
    echo $poruka;
	echo "</table>";
	// Figure out the total number of results, DISPLAY last 20 events 
