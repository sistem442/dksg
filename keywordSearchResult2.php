...
$queryNum = "SELECT COUNT(*) as Num FROM event WHERE";
$reci = explode(" ", $rec);
// ako je u polje uneto vise reci iseci ih u pojedinacne reci i napravi od njih niz
for($j=0; $j<sizeof($reci);$j++){
	$query = $query." (tekst LIKE '%".$reci[$j]."%') AND";
	$queryNum = $queryNum." (tekst LIKE '%".$reci[$j]."%' ) AND";
	//za svaku pojedinacnu rec u nizu, dodaj parametar i vrednost u upit
}
$query = substr($query, 0, -4);
$queryNum = substr($queryNum, 0, -4);
// za zadnju rec izbaci " AND"
echo $query."<br/>";
echo $queryNum."<br/>";
...
//SQL izraz
$query =$query." ORDER BY godina DESC, mesec DESC, dan ASC LIMIT $from, $max_results ;";
echo $query;
...
$total_results = mysql_result(mysql_query($queryNum),0);  
