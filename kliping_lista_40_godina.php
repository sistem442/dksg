...
if(isset($_POST['submit'])){
	...
	//pretraga samo po recima
		...
		$query = ("SELECT * FROM kliping 
					WHERE  ".$upit." AND kategorija = '40' 
					ORDER BY `kliping`.`godina_unosa` ASC, `kliping`.`mesec_unosa` 
					ASC,`kliping`.`dan_unosa` ASC LIMIT 0, 30 ;");
	
		//echo '<br>greska:'.mysql_errno($con) . ": " . mysql_error($con) . "\n";
		//echo '<br>Izraz za upis u bazu:'.$query;
}

//If form is not submited, display form and selected cliping

else $query = ("SELECT * FROM kliping WHERE kategorija='40' ORDER BY `kliping`.`godina_unosa` ASC, `kliping`.`mesec_unosa` ASC,`kliping`.`dan_unosa` ASC LIMIT 0, 100 ;");
            ...
            <td>
                <select id="p_godina" name="godina" class="text_box">
                    <option value="" selected>G</option>
                     <?php echo $select1->get_select_year_from(1970);?>          
                </select>
            </td>
        </tr>
