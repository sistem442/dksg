...
<?php 
	...
	$i=date("n");
  	...
  	$redakcije = array("Forum","Pozorište","Književnost","Likovni program","Muzika","Film","Biblioteka");
	$kreator = array("milos","lada","Tamara","maida","milan.d","nemanja","nevena");
	
	//pocetak arhive
	echo '<div class="arrowlistmenu" style="width:670px">';
			
			//lista mesece
			for($x = 12;$x>0;$x--){
				//echo $i.' '.$x;
				if($i == 0){$i = 12; $godina22 = $godina22 -1;}
...
<?php
				...
				//lista redakcije za odredjeni mesec
				for($j=0;$j<7;$j++){					
					...
					echo "<li><a href='#' class='subexpandable'>".$redakcije[$j]." (".$brojrezultata.")</a>
						...
						//lista dogadjaje za odredjenu redakciju	
							...
							if( is_null($row['s_opis'])){
								$program = $row['program'];
								  switch($program)
								  {
									...
									$outup_row = $dan.'.'. $mesec.'.'.$row['godina'].' |
										 '.$program2.' | '.$row['imePrograma'];
									}
									else{
										$outup_row = $dan.'.'. $mesec.'.'.$row['godina'].' | '.$row['s_opis'];
										}
									?>
									<li><a  href='oneEventDisplay.php?id=
											<?php echo ($row['id']);?>'><?php echo $outup_row;?> </a></li>
									<?php 
					 }echo '</ul>
					 </li>
					 '; //kraj petlje koja koja lista dogadjaje koje su uneli urednici
						 ...
						 //Listanje dogadjaj koja je uneo afc	
				$query2 = ("SELECT event.id,event.s_opis, event.dan,event.mesec,event.godina,event.imePrograma, event.program FROM event INNER JOIN korisnici ON korisnici.username = event.kreator WHERE mesecUnosa =".$i." AND (podgrupa = 'afc') AND event.godinaUnosa = ".$godina22." ORDER BY event.id DESC");
				//echo $query2;
				//echo mysql_errno($con) . ": " . mysql_error($con) . "\n";
				$result2 = mysql_query($query2);
				$brojrezultata2 = mysql_num_rows($result2);
				echo "<li><a href='#' class='subexpandable'>AFC (".$brojrezultata2.")</a>
						<ul class='subcategoryitems' style='margin-left: 15px'> ";
...
<?php	}echo '</ul>
						 ...
						 ';//kraj petlje koja koja lista dogadjaje koja je uneo afc		
						 //Listanje dogadjaj koja je unela arhiva	
